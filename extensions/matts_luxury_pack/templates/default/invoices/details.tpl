...
{/if}
					...
					<td{if !$invoice.ship_to_customer_id} class="si_hide" xstyle="display: none;"{/if} id="ship_to_btns">
						<a class="index_table" href="index.php?module=customers&view=details&id={$invoice.ship_to_customer_id}&action=view" title="{$LANG.view}">
							<img src="images/common/view.png" height="16" alt="view" /><!--{$LANG.view}--></a><!-- border="-5px" padding="-4px"-->
						<a class="index_table" href="index.php?module=customers&view=details&id={$invoice.ship_to_customer_id}&action=edit" title="{$LANG.edit}">
							<img src="images/common/edit.png" height="16" alt="edit" /><!--{$LANG.edit}--></a><!-- border="-5px" padding="-4px"-->
						<a class="index_table" title="{$LANG.print_preview}" href="index.php?module=export&view=deliverynote&id={$invoice.ship_to_customer_id}&format=print">
							<img src="images/common/printer.png" height="16" alt="print" /><!-- {$LANG.print_preview}print --></a><!-- border="-5px" padding="-4px"-->
					<!--3 EXPORT DIALOG -->
						<a title="{$LANG.export}" class="invoice_export_dialog" href="#" rel="{$invoice.ship_to_customer_id}">
							<img src='images/common/page_white_acrobat.png' class='action' height="16" alt="acrobat" /><!--{$LANG.export}--></a><!-- border="-5px" padding="-4px"-->
					...
					<!--3 EXPORT TO PDF 
						<a title='Export' class='index_table' href='pdfmaker.php?id={$invoice.ship_to_customer_id}'>
							<img src='images/common/page_white_acrobat.png' height="16" alt="acrobat" /></a> --><!-- border="-5px" padding="-4px"-->
					<!--4 XLS 
						<a title='Export to a spreadsheet asxls format' class='index_table' href='index.php?module=invoices&view=templates/template&invoice={$invoice.ship_to_customer_id}&action=view&location=print&export=xls'>
							<img src='images/common/page_white_excel.png' height="16" alt="excel" /></a> --><!-- border="-5px" padding="-4px"-->
					<!--7 Email -->
						<a title="{$LANG.email}" href='index.php?module=invoices&view=email&stage=1&id={$invoice.ship_to_customer_id}'>
							<img src='images/common/mail-message-new.png' height="16" alt="email" /></a><!-- border="-5px" padding="-4px" valign="bottom"-->
					</td>
				</tr>
<!-- end Ship To -->
...
{if $invoice.type_id == 1}
				...
				<tr>
					<td class='si_invoice_notes' colspan="2">
						...
						<textarea class="editor" name="description0" rows="10" cols="70" wrap="nowrap">{$invoiceItems.0.description|outhtml}</textarea>
					</td>
					<td>&nbsp;</td>
				</tr>		
...
{/section}
				...
				<tr class="details si_hide">
					...
					<td colspan="4">
						<textarea class="detail" name="description{$line|htmlsafe}" id="description{$line|htmlsafe}" rows="3" cols="3" wrap="nowrap">{$invoiceItem.description}</textarea>{*outhtml*}
					</td>
				</tr>
			</tbody>
{/foreach}
		...
		<div class="si_toolbar si_toolbar_inform">
			...
			<a href='#' class="details si_hide" onclick="javascript: $('.details').hide();$('.show-details').show();" xstyle="display:none"><img src="./images/common/page_white_delete.png" title="{$LANG.hide_details}" alt="hide" />{$LANG.hide_details}</a>
		</div>
		...
		{*	{showCustomFields categorieId="4" itemId=$smarty.get.invoice}	*}
			<tr>
				<td class='si_invoice_notes' colspan="2">
					<H5>{$LANG.notes}</H5>
					<textarea class="editor" name="note" rows="10" cols="70">{$invoice.note|outhtml}</textarea><!--input type="text"  wrap="nowrap"-->
				</td>
			</tr>		
{/if}
