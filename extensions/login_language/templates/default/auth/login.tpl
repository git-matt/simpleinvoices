...
{/if}
			...
			<div class="si_box">
				...
				<div class="si_box_auth_pad">
					<table>
						<tr>
							<th id="email">{$LANG.email}</th>
							<td><input name="user" size="25" type="text" title="user" value="" /></td>
						</tr>
						<tr>
							<th id="password">{$LANG.password}</th>
...
{if $smarty.capture.hook_auth_login_table_end}{$smarty.capture.hook_auth_login_table_end}{/if}
					</table>
...
{/if}
	...
	{if $smarty.capture.hook_auth_login_end}{$smarty.capture.hook_auth_login_end}{/if}
</body>
</html>